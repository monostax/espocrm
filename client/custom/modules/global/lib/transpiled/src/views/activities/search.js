define("global:views/activities/search", ["exports", "views/record/search", "model"], function (_exports, _search, _model) {
  "use strict";
   ...
   */
  class ActivitiesSearchView extends _search.default {
    setup() {
      ...
      this.collection.prepareModel = () => {
        const fields = metadata.get(["entityDefs", "Activities", "fields"]) || {};
        const ActivityModel = _model.default.extend({
        ...
        model.getFieldType = function (field) {
          var _this$defs;
          return metadata.get(["entityDefs", "Activities", "fields", field, "type"]) || ((_this$defs = this.defs) === null || _this$defs === void 0 || (_this$defs = _this$defs.fields) === null || _this$defs === void 0 || (_this$defs = _this$defs[field]) === null || _this$defs === void 0 ? void 0 : _this$defs.type);
        };
        model.getFieldParam = function (field, param) {
          var _this$defs2;
          return metadata.get(["entityDefs", "Activities", "fields", field, param]) || ((_this$defs2 = this.defs) === null || _this$defs2 === void 0 || (_this$defs2 = _this$defs2.fields) === null || _this$defs2 === void 0 || (_this$defs2 = _this$defs2[field]) === null || _this$defs2 === void 0 ? void 0 : _this$defs2[param]);
        };
        model.getLinkParam = function (link, param) {
          return metadata.get(["entityDefs", "Activities", "links", link, param]);
        };
        model.hasLink = function (link) {
          return !!metadata.get(["entityDefs", "Activities", "links", link]);
        };
        return model;
      };
      ...
      const filterList = metadata.get(["clientDefs", "Activities", "filterList"]) || [];
      const forbiddenFieldList = this.getAcl().getScopeForbiddenFieldList(this.entityType) || [];
      console.log("ActivitiesSearchView: filterList from clientDefs:", filterList);
      console.log("ActivitiesSearchView: forbiddenFieldList:", forbiddenFieldList);
      this.fieldFilterList = filterList.filter(field => !forbiddenFieldList.includes(field));
      console.log("ActivitiesSearchView: final fieldFilterList:", this.fieldFilterList);
     ...
     */
    fetch() {
      console.log("fetch() called, advanced fields:", Object.keys(this.advanced || {}));
      ...
      for (const field in this.advanced) {
        ...
        if (searchData === false || searchData === null || searchData === undefined) {
          console.log(`Removing ${field} - fetchSearch returned falsy value`);
          delete this.advanced[field];
          continue;
        }

        // If it's an object, check if it has a type
        if (typeof searchData === "object" && (!searchData.type || searchData.type === "")) {
    ...
    updateSearch() {
      ...
      for (const field in this.advanced) {
        ...
        if (!data || typeof data !== "object" || !data.type || data.type === "") {
  ...
  var _default = _exports.default = ActivitiesSearchView;
});
