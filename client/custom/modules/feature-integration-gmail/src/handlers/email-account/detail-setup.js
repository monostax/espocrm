...
/**
 * EmailAccount detail/edit setup for Gmail OAuth.
 *
 * When a Gmail OAuthAccount is selected in the UI (before save), pre-fill
 * Gmail IMAP/SMTP hosts so the form matches what the server-side hook will
 * persist. Does not overwrite non-Gmail hosts the user already set.
 */
define("feature-integration-gmail:handlers/email-account/detail-setup", [], () => {
	...
	return class {
		...
		applyGmailDefaults(model) {
			...
			// Clear password fields in the form when switching to OAuth.
			model.set({
				password: null,
				smtpPassword: null,
			});
