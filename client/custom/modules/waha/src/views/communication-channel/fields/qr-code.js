...
define('waha:views/communication-channel/fields/qr-code', ['views/fields/text'], function (Dep) {

    return Dep.extend({

        detailTemplate: 'waha:communication-channel/fields/qr-code/detail',
        ...
        loadQrCode: function () {
            ...
            this.$el.find('.qr-code-container').html(
                ...
                this.translate('Loading QR Code', 'labels', 'CommunicationChannel') +
                '</div>'
            );

            Espo.Ajax.getRequest('CommunicationChannel/action/qrCode', { id: id })
                .then(response => {
                    if (response && response.dataUrl) {
                        this.$el.find('.qr-code-container').html(
                            '<img src="' + response.dataUrl + '" alt="QR Code" class="qr-code-image" />' +
                            '<p class="qr-code-hint">' + 
                            this.translate('Scan QR Code with WhatsApp', 'labels', 'CommunicationChannel') +
                            '</p>' +
        ...
        checkStatusAndRefresh: function () {
            ...
            Espo.Ajax.postRequest(`CommunicationChannel/${id}/checkStatus`)
                .then(response => {
                    if (response.status !== this.model.get('status')) {
                        this.model.set(response);
                        if (response.status === 'ACTIVE') {
                            Espo.Ui.success(this.translate('channelConnected', 'messages', 'CommunicationChannel'));
                        }
