...
define('chatwoot:views/chatwoot-conversation/modals/conversation-drawer', ['views/modal'], function (Dep) {
    return Dep.extend({
        ...
        setup: function () {
            this.headerText = this.options.contactName || this.translate('Conversation', 'scopeNames');
            ...
            if (!this.hasConversation) {
                this.errorMessage = !chatwootAccountId 
                    ? this.translate('Your user is not linked to a Chatwoot account', 'messages', 'ChatwootConversation')
                    : this.translate('No conversation ID available', 'messages', 'ChatwootConversation');
                return;
            }
            ...
            const hasSsoAuthenticated = sessionStorage.getItem(this.CHATWOOT_SSO_AUTH_KEY) === 'true';
            
            if (chatwootSsoUrl && !hasSsoAuthenticated) {
                // Use SSO URL for first-time authentication
                this.chatwootUrl = chatwootSsoUrl;
        ...
        afterRender: function () {
            ...
            if ($iframe.length) {
                const updateHeight = () => {
                    ...
                    const footerHeight = $modal.find('.modal-footer').outerHeight() || 0;
                    const windowHeight = $(window).height();
                    const availableHeight = windowHeight - footerHeight;
                    $iframe.css('height', Math.max(300, availableHeight) + 'px');
                };
        ...
        actionViewDetails: function () {
            ...
            if (this.recordId) {
                this.getRouter().navigate('#ChatwootConversation/view/' + this.recordId, {trigger: true});
            }
        },
    });
});
