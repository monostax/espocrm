...
define("google:views/google/google", [
    "views/external-account/oauth2",
    "model",
], function (Dep, Model) {
    return Dep.extend({
        ...
        setup: function () {
            ...
            if (
                this.getLanguage().has(
                    this.integration,
                    "help",
                    "ExternalAccount"
                )
            ) {
                this.helpText = this.translate(
                    this.integration,
                    "help",
                    "ExternalAccount"
                );
            ...
            var products = this.getMetadata().get(
                "integrations.Google.products"
            );
            ...
            this.listenToOnce(this.model, "sync", () => {
                Espo.Ajax.getRequest(
                    `ExternalAccount/action/getOAuth2Info?id=${this.id}`
                ).then((response) => {
            ...
            this.popup(
                {
                    path: this.getMetadata().get(
                        "integrations." + this.integration + ".params.endpoint"
                    ),
                    params: {
                        client_id: this.clientId,
                        redirect_uri: this.redirectUri,
                        scope: this.getMetadata().get(
                            "integrations." + this.integration + ".params.scope"
                        ),
                        response_type: "code",
                        access_type: "offline",
                ...
                (res) => {
                    ...
                    if (res.code) {
                        ...
                        Espo.Ajax.postRequest(
                            "ExternalAccount/action/authorizationCode",
                            {
                                id: this.id,
                                code: res.code,
                            }
