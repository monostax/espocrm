...
/**
 * IMAP storage handler for Gmail via OAuthAccount (XOAUTH2).
 *
 ...
 */
class GmailImapHandler implements Handler
{
    ...
    public function handle(Params $params, string $id): Params
    {
        ...
        if ($entity === null) {
            throw new RuntimeException("GmailImapHandler: account '$id' not found.");
        }

        $oAuthAccountId = $entity->get('oAuthAccountId');

        if (!$oAuthAccountId) {
            // Handler is set but link was cleared — fall through with stored params
            // (password auth). Callers that expect OAuth should re-save the account.
            $this->log->warning(
                "GmailImapHandler: account '$id' has no oAuthAccountId; using stored credentials."
            );
        ...
        try {
            ...
            $this->log->error(
                "GmailImapHandler: failed to obtain token for OAuthAccount '$oAuthAccountId' " .
        ...
        if (!$emailAddress) {
            throw new RuntimeException("GmailImapHandler: no email address on account '$id'.");
        }
