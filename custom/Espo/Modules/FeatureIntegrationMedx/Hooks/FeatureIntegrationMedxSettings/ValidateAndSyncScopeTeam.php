<?php
namespace Espo\Modules\FeatureIntegrationMedx\Hooks\FeatureIntegrationMedxSettings;
...
class ValidateAndSyncScopeTeam
{
    ...
    public function beforeSave(Entity $entity, array $options): void
    {
        ...
        $this->assertCredentialType($entity);
    ...
    private function assertExactlyOneScopeTeam(Entity $entity): string
    {
        ...
        if (count($teamsIds) !== 1) {
            throw new BadRequest('Each MEDX integration profile must have exactly one scope team.');
        }
        $entity->set('teamsIds', $teamsIds);
        return $teamsIds[0];
    }
    private function assertCredentialType(Entity $entity): void
    {
        ...
        if (!$webCredentialId) {
            throw new BadRequest('Web credential is required for MEDX integration profiles.');
        }
        $webCredential = $this->entityManager->getEntityById('Credential', $webCredentialId);
        if (!$webCredential) {
            throw new BadRequest('Web credential must exist.');
        }
    ...
    private function assertNoOtherActiveProfileForScopeTeam(Entity $entity, string $scopeTeamId): void
    {
        ...
        $repository = $this->entityManager->getRDBRepository('FeatureIntegrationMedxSettings');
        $where = [
            'isActive' => true,
        ...
        $query = $this->entityManager
            ->getQueryBuilder()
            ->select(['id'])
            ->from('FeatureIntegrationMedxSettings')
            ->join('teams', 'teams')
            ->where($where);
        $existing = $repository->clone($query->build())->findOne();
        if ($existing) {
            throw new BadRequest(
                'There is already an active MEDX integration profile for this scope team. '
    ...
    private function syncDefaultName(Entity $entity): void
    {
        ...
        $entity->set('name', $tenantName ? "{$tenantName} - MEDX Profile" : 'MEDX Profile');
    }
