...
| **enrollToWhatsAppCampaign** | tenant | Enroll journey **Contact** into a running WhatsApp Campaign (`status = Sending`). Params: `whatsAppCampaignId` (required), optional `phone` override. Creates Pending `WhatsAppCampaignContact` rows (one per sendable phone) and schedules campaign chunk jobs via `WhatsAppCampaignService::enrollAudience`. Idempotent by phone. Soft-skips opt-out / no phone. Template & inbox come from the campaign. |
