...
class BackfillOpportunityFollowupAiAgentId implements RebuildAction
{
    ...
    public function process(): void
    {
        ...
        try {
            ...
            //
            // MySQL's multi-table `UPDATE ... JOIN ... SET` has no direct
            // PostgreSQL equivalent, so the statement is branched per
            // driver. The MySQL string is kept byte-identical to the
            // original to guarantee zero behavior change on the
            // production MySQL/MariaDB tenant; the PostgreSQL branch is
            // the semantically equivalent `UPDATE ... FROM` form.
            if ($isPg) {
                $stmt = $pdo->prepare(
                    "UPDATE opportunity o "
                    . "SET followup_ai_agent_id = COALESCE("
                    .   "inbox_agent.membership_id, account_agent.membership_id"
                    . ") "
                    . "FROM chatwoot_conversation_opportunity cco "
                    . "JOIN chatwoot_conversation cc "
                    .   "ON cc.id = cco.chatwoot_conversation_id AND cc.deleted = false "
                    . "JOIN chatwoot_inbox ci "
                    .   "ON ci.chatwoot_inbox_id = cc.chatwoot_inbox_id AND ci.deleted = false "
                    . "JOIN chatwoot_account ca "
                    .   "ON ca.id = ci.chatwoot_account_id AND ca.deleted = false "
                    . "LEFT JOIN ("
                    .   "SELECT "
                    ...
                    .     "ON m.chatwoot_user_id = u.id AND u.deleted = false "
                    .   "JOIN chatwoot_account_user_membership_chatwoot_inbox mi "
                    .     "ON mi.chatwoot_account_user_membership_id = m.id AND mi.deleted = false "
                    .   "JOIN chatwoot_inbox cii "
                    .     "ON mi.chatwoot_inbox_id = cii.id AND cii.deleted = false "
                    .   "WHERE m.is_a_i = true AND m.deleted = false"
                    . ") inbox_agent "
                    ...
                    . "WHERE cco.opportunity_id = o.id "
                    . "AND cco.deleted = false "
                    . "AND o.followup_status = 'FollowupActive' "
