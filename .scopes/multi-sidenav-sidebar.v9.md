# Multi-Sidenav Sidebar Mode - Implementation Plan v9

> **Version**: 9.0  
> **Based on**: `.scopes/multi-sidenav-sidebar.v8.md` and `.scopes/multi-sidenav-sidebar.v8.audit.md`  
...
## Audit Corrections Applied (v8 → v9)

### Critical Findings Addressed

| Issue | v8 Problem | v9 Correction |
|-------|-----------|---------------|
| navbar-config-list.js HTML Pattern | Uses legacy string concatenation with `escapeString()` | Rewrite `getGroupItemHtml()` using `document.createElement()` pattern matching `tab-list.js:138-235` |
| Preferences/detail.json Structure | Plan shows inserting `{ "rows": [...] }` which creates invalid JSON | Append rows to the existing section at lines 111-122, not create a new wrapped object |
| Settings.json JSON Structure | Insert "after line 255" without proper comma handling | Insert between lines 255-256, ensuring comma after `tabList` closing brace and after new fields |
| active-navbar-config.js Events | Event pattern for config changes needs verification | Verify `'change:navbarConfigList'` event on `this.getConfig()` works, or use helper's settings sync event |

### Warning Items Addressed

| Warning | v9 Required Fix |
|---------|----------------|
| navbar-config-selector.js events definition | Move `events` definition to class property level, or keep `addActionHandler` pattern consistently |
| Missing translation keys for modal headers | Add `"Edit Navbar Configuration"` and `"Add Navbar Configuration"` to Settings.json i18n under "labels" |
| Global.json i18n comma handling | Ensure comma is added after `navbarTabs` closing brace before adding `navbarConfig` section |
| Error message translation key | Verify `errorSavingPreference` insertion location in Global.json messages section |
| switchNavbarConfig error handling | Add specific handling for server-side validation errors |
...
| Portal Support | **Out of scope** for v9 | Portal has separate `tabList` system; can be added later |
...
| `client/src/views/preferences/fields/active-navbar-config.js` | Dropdown field to select active config. Must use verified config change event pattern. |
...
| `application/Espo/Resources/layouts/Preferences/detail.json` | Add navbar config fields to User Interface section | **CRITICAL AUDIT FINDING**: Append rows to the existing section at lines 111-122, NOT create a new wrapped object with `{ "rows": [...] }`. The structure is an array of objects, not nested. |
...
|-----------|--------|------------------|
| `client/src/views/site/navbar.js` | Multiple edits: (A) Call `setupNavbarConfigSelector()` after line 461, (B) Update preferences listener at lines 466-478 to include new fields, (C) Add new methods `setupNavbarConfigSelector()` and `switchNavbarConfig()` AFTER line 1045 (after `adjustAfterRender()` closes, before `selectTab()`) | **CRITICAL**: Verify config change event pattern - use `'change:navbarConfigList'` on `this.getConfig()` |
...
| File Path | Reason |
|-----------|--------|
| `application/Espo/Resources/metadata/entityDefs/Portal.json` | Portal has separate tabList system - explicitly **OUT OF SCOPE** for v9 |
| `client/src/views/portal/navbar.js` | Portal navbar - explicitly **OUT OF SCOPE** for v9 |
...
*Scope document v9 generated with all v8 audit corrections applied - SCOPE MAPPED*
