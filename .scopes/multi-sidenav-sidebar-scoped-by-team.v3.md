# Multi-Sidenav Sidebar Scoped by Team - Implementation Scope v3

> **Version**: 3.0  
> **Based on**: v2 scope + v2.audit findings  
...
### Key Changes from v2
- **Warning Fix**: Added explicit `links` section structure to entity definition
- **Warning Fix**: Added explicit `detailSmall.json` layout structure
- **Warning Fix**: Changed Preferences layout reference to pattern-based description
- **Suggestion Adopted**: Added `foreign` relationship to team link for bidirectional navigation
- **Suggestion Adopted**: Added BeforeSave hook specification for `isDefault` validation
...
#### 2. Entity Definition (CRITICAL - v3: Explicit links section)

| File Path | Purpose |
|-----------|---------|
| `custom/Espo/Modules/Global/Resources/metadata/entityDefs/SidenavConfig.json` | New entity for team-scoped navbar configurations. Pattern follows `Funnel.json` and `ChatwootInboxIntegration.json`. |
...
**Reference Pattern:** `custom/Espo/Modules/Global/Resources/metadata/entityDefs/Funnel.json` (lines 46-70 for links structure)
...
#### 5. BeforeSave Hook for isDefault Validation (NEW IN v3)
...
**detailSmall.json structure (NEW IN v3 - explicit structure):**
```json
[
    {
        "label": "Overview",
...
#### 12. Configurations Translations

| File Path | Purpose |
|-----------|---------|
| `custom/Espo/Modules/Global/Resources/i18n/en_US/Configurations.json` | Panel labels for adminForUser. Used by `admin-for-user/index.js`. |
...
**Current content** (lines 1-20) contains the fields to delete. Replace entire content with:
```json
{
    "fields": {
...
**Current content** (lines 1-20) - replace entire content with:
```json
{
    "fields": {
...
| File Path | Changes |
|-----------|---------|
| `custom/Espo/Modules/Global/Resources/layouts/Settings/userInterface.json` | DELETE rows containing `navbarConfigList`, `navbarConfigDisabled`, `navbarConfigSelectorDisabled`. ADD row with `navbarConfigShowDefaultTabList`. |

**Reference:** Locate the Navbar section in the layout and add the new field.

#### 4. Preferences Layout (v3: Pattern-based reference)

| File Path | Changes |
|-----------|---------|
| `custom/Espo/Modules/Global/Resources/layouts/Preferences/detail.json` | DELETE the section containing `useCustomNavbarConfig` and `navbarConfigList` rows. KEEP `activeNavbarConfigId` - move to User Interface section. |

**Pattern-based location:** Within the "User Interface" tab section (after the tabList rows), there is a block containing:
- `useCustomNavbarConfig` row
- `navbarConfigList` row (with fullWidth)
- `activeNavbarConfigId` row

**Action:**
- DELETE the rows containing `useCustomNavbarConfig` and `navbarConfigList`
- KEEP `activeNavbarConfigId` - it should remain in the User Interface section

**Result:** The User Interface section should have the `activeNavbarConfigId` row after the `tabList` row.
...
3. Create `SidenavConfig.json` entity definition (with explicit links section)
4. Create `SidenavConfig` scope definition
5. Create `SidenavConfig` clientDefs metadata
6. Create `EnsureSingleDefault.php` BeforeSave hook
...
*v3 scope document generated addressing all warnings and suggestions from v2.audit*
