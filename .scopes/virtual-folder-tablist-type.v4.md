# Virtual Folder Tab List Type - v4 Scope Document
...
Based on v3 audit findings, v4 addresses:

1. **CRITICAL: Virtual folder config data flow** - `prepareVirtualFolderDefs()` now passes the full `tab` object as `config` property to `createVirtualFolderView()`
2. **WARNING: editGroup() extension details** - Explicit implementation with virtual folder routing to custom modal
3. **WARNING: Template structure alignment** - Virtual folder renders as `<li class="virtual-folder">` to maintain navbar `<ul>` structure
4. **WARNING: SidenavConfig integration** - Confirmed `jsonArray` field type supports virtual folder objects
5. **WARNING: Default virtual folder object** - Explicit default object structure for add modal
6. **SUGGESTION: View All action** - Added `actionViewAll()` implementation
7. **SUGGESTION: Error handling** - Added error state with template support
...
- **v4 FIX: Receives full config via `this.options.config`** containing the complete tab object
...
- Creates quick create view using `RecordModal` helper with **after:save refresh**:
  ...
  ```
- **v4 ADD: View All action implementation:**
  ```javascript
  actionViewAll() {
...
3. **Override `prepareTabItemDefs()` to handle virtual folders:**
   ...
   // v4 FIX: Pass the full tab object as config property
   prepareVirtualFolderDefs(params, tab, i, vars) {
       ...
       return {
           ...
           config: tab,  // v4 FIX: Pass the full tab config for use in createVirtualFolderView
...
4. **Override `setupTabDefsList()` to create virtual folder views AND handle filtering:**
   ...
   // v4 FIX: Receive config from defs (which comes from prepareVirtualFolderDefs)
   createVirtualFolderView(defs) {
       ...
       // v4 FIX: defs.config now contains the full tab configuration
...
**v4 FIX: Renders as `<li class="virtual-folder">` to maintain navbar structure.**
...
**Note:** v4 fix - Template now renders as `<li>` element to maintain proper navbar `<ul>` structure.
...
#### 6. Tab List Field View Extension (MEDIUM - v4 EXPLICIT)
...
**v4 FIX: Explicit editGroup() implementation with virtual folder routing.**
...
export default class CustomTabListFieldView extends TabListFieldView {
    ...
    /**
     * v4 ADD: Get HTML for virtual folder item display.
     */
    ...
    /**
     * v4 FIX: Explicit editGroup() override with virtual folder routing.
     */
    editGroup(id) {
        ...
        // v4 FIX: Add virtual folder to the view map
        const view = {
...
#### 10. Tab List Field Add Modal Extension (LOW - v4 EXPLICIT)
...
**v4 FIX: Explicit default virtual folder object structure.**
...
export default class CustomTabListFieldAddModal extends TabListFieldAddSettingsModalView {
    ...
    /**
     * v4 FIX: Explicit default virtual folder object with all required fields.
     */
...
### Fetch Error
- **v4 ADD:** Show "Failed to load" message in virtual folder with retry link
...
*v4 Scope document - Addresses all v3 audit findings: CRITICAL config data flow fix, explicit editGroup() implementation, template as `<li>` element, verified SidenavConfig compatibility, explicit default object, viewAll action, error handling.*
