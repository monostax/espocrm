# Virtual Folder - Open Relationship View - v3 Scope Document

> **Version**: 3.0  
> **Codebase Root**: `components/crm/source/`  
> **Status**: READY FOR IMPLEMENTATION  
> **Uses**: Global module (`custom/Espo/Modules/Global` and `client/custom/modules/global`)
> **Revises**: v2 - Addresses audit findings from `virtual-folder-open-relationship-view.v2.audit.md`
...
| 14 | **Use `showValidationMessage()` for field validation** | `setValidationMessage()` | Correct EspoCRM API method name (verified in `client/src/views/fields/base.js`) |
...
**IMPORTANT:** Use `showValidationMessage(message)` - NOT `setValidationMessage(type, message)`. This is the correct EspoCRM API (verified in `client/src/views/fields/base.js` line 1546).
...
| `client/src/views/fields/base.js` | `showValidationMessage(message, target, view)` method (line 1546) |
...
2. Edit `views/settings/modals/edit-tab-virtual-folder.js`:
   ...
   - Add custom validation in `actionApply()` using `showValidationMessage()`
...
| # | Change | Reason |
|---|--------|--------|
| 1 | Decision 14: Use `showValidationMessage()` | Correct EspoCRM API method name (Warning from v2 audit) |
| 2 | Added console.warn for missing field view | Defensive programming (Suggestion from v2 audit) |
| 3 | Added `base.js` to reference files | Documents the `showValidationMessage` source |
...
*v3 Scope document - Ready for implementation. Addresses all audit findings from v2.*
