...
module.exports = (grunt) => {
    ...
    for (const name in bundleConfig.chunks) {
        ...
        bundleFileMap[`client/lib/${namePart}.js`] =
            originalLibDir + `/${namePart}.js`;
    }
    ...
    fs.readdirSync("application/Espo/Resources/metadata/themes").forEach(
        (file) => {
    ...
    themeList.forEach((theme) => {
        ...
        files["client/css/espo/" + name + ".css"] =
            "frontend/less/" + name + "/main.less";
        files["client/css/espo/" + name + "-iframe.css"] =
            "frontend/less/" + name + "/iframe/main.less";

        cssminFilesData["client/css/espo/" + name + ".css"] =
            "client/css/espo/" + name + ".css";
        cssminFilesData["client/css/espo/" + name + "-iframe.css"] =
            "client/css/espo/" + name + "-iframe.css";
    ...
    grunt.initConfig({
        ...
        clean: {
            transpiled: ["client/lib/transpiled/**"],
            start: [
                "build/EspoCRM-*",
                "client/lib/*",
    ...
    grunt.registerTask("bundle-templates", () => {
        const templateBundler = new TemplateBundler({
            dirs: ["client/res/templates", "client/modules/crm/res/templates"],
        });
    ...
    grunt.registerTask("chmod-multiple", () => {
        ...
        dirReadableList.forEach((item) => {
            cp.execSync(`chmod 755 ${item}`, {
                cwd: dirPath,
            });
        });

        dirWritableList.forEach((item) => {
            cp.execSync(`chmod 775 ${item}`, {
                cwd: dirPath,
            });
        });

        cp.execSync(`find bin -type f -exec chmod 755 {} +`, {
            cwd: dirPath,
        });
    });
    ...
    grunt.registerTask("integration-tests-run", () => {
        cp.execSync("vendor/bin/phpunit ./tests/integration", {
            stdio: "inherit",
        });
    });
    ...
    grunt.registerTask("generate-build-version", () => {
        const now = new Date();
        ...
        try {
            commitHash = cp.execSync('git rev-parse --short HEAD', { encoding: 'utf8' }).trim().substring(0, 6);
        } catch (e) {
            // Git not available or not a git repo
        }

        const version = `${year}.${month}.${day}.${hour}${minute}.${commitHash}`;
...
`;
        ...
        const dir = 'application/Espo/Resources/defaults';
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true });
        }

        fs.writeFileSync(dir + '/build-version.php', content, 'utf8');
        console.log(`Build version generated: ${version}`);
    });

    grunt.registerTask("zip", function () {
        // Don't change to arrow-function.
    ...
    grunt.registerTask("release", ["build", "upgrade", "zip", "clean:release"]);
